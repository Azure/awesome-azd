...
const { extensionBranchName, escapeMarkdown, applyCap, validateExtensionEntry } = require("./github-utils");
...
module.exports = async ({ github, context }) => {
  ...
  for (const item of candidates) {
    ...
    // Validate entry schema before creating branch/PR
    if (!validateExtensionEntry(entry)) {
      console.error(`Skipping ${extensionId}: entry missing required fields`);
      continue;
    }
    ...
    try {
      ...
      const safeInstallCommand = escapeMarkdown(`azd ext install ${extensionId}`);
      ...
      // Add labels via issues API (pulls.create doesn't support labels param)
...
| Field | Value |
|-------|-------|
| **Repository** | [${escapeMarkdown(repoFullName)}](${repoUrl}) |
