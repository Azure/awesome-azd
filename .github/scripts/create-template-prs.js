...
const { templateBranchName, escapeMarkdown, applyCap, validateTemplateEntry } = require("./github-utils");
...
module.exports = async ({ github, context }) => {
  ...
  for (const item of candidates) {
    ...
    if (!validateTemplateEntry(entry)) {
      console.error(`Skipping ${repoFullName}: entry missing required fields`);
      continue;
    }
    ...
    const branchName = templateBranchName(owner, repo);
